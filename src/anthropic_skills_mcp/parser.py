    ...
    try:
        ...
        try:
            ...
            if not name or not description:
                logger.error(
                    f"Skill file {file_path} missing 'name' or 'description' in frontmatter"
                )
                return None
...
def load_skills(skills_dir: str) -> Dict[str, Skill]:
    ...
    if not os.path.isdir(skills_dir):
        logger.error(
            f"Skills directory {skills_dir} does not exist or is not a directory"
        )
        return skills
            ...
            if file == "SKILL.md":
                ...
                if skill:
                    if skill.name in skills:
                        logger.warning(
                            f"Duplicate skill name '{skill.name}' found at {full_path}. Skipping."
                        )
                        continue
